...
// === HELPER FUNCTION: Registration Code Generator ===
function generate_registration_code($length = 7) {
    // Use A-Z and 0-9 for clear, easily readable codes
    $characters = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    $code = '';
    for ($i = 0; $i < $length; $i++) {
        $code .= $characters[rand(0, strlen($characters) - 1)];
    }
    // Prefix the code for easy identification (e.g., REG-XXXXXXX)
    return "REG-" . $code; 
}
// === END HELPER FUNCTION ===
...
if ($action === 'register' && $method === 'POST') {
    ...
    if ($stmt->execute()) {
        ...
        // 1. DELETE the used code from the database
        $stmt_delete = $conn->prepare("DELETE FROM registration_codes WHERE code = ?");
        if (!$stmt_delete) {
            error_log("CRITICAL: Failed to delete used registration code '{$reg_code}': " . $conn->error);
        } else {
            $stmt_delete->bind_param("s", $reg_code);
            $stmt_delete->execute();
            $stmt_delete->close();
        }
        
        // 2. Generate and insert a new unique active 7-digit registration code
        $new_code = '';
        $is_unique = false;
        $NEW_CODE_LENGTH = 7; 

        // Loop up to 10 times to ensure the code is unique
        for ($i = 0; $i < 10 && !$is_unique; $i++) {
            $new_code = generate_registration_code($NEW_CODE_LENGTH);
            
            // Check for collision in the database
            $stmt_check = $conn->prepare("SELECT 1 FROM registration_codes WHERE code = ?");
            $stmt_check->bind_param("s", $new_code);
            $stmt_check->execute();
        ...
        if ($is_unique) {
            $stmt_insert = $conn->prepare("INSERT INTO registration_codes (code, is_active) VALUES (?, 1)");
            if (!$stmt_insert) {
                error_log("CRITICAL: Failed to generate/insert new registration code: " . $conn->error);
            } else {
                $stmt_insert->bind_param("s", $new_code);
                $stmt_insert->execute();
                $stmt_insert->close();
            }
        } else {
             error_log("CRITICAL: Failed to generate a unique code after 10 attempts.");
        }
...
// --- NEW ACTION: Update User Password (Admin Only) ---
} elseif ($action === 'updatePassword' && $method === 'POST') {
    check_admin_auth($conn);

    $user_id = $data['userId'] ?? null;
    $new_pass = trim($data['newPassword'] ?? '');

    if (empty($user_id) || empty($new_pass) || strlen($new_pass) < 6) {
        respond_error($conn, "Invalid user ID or new password (min 6 chars required).", 400);
    }
    ...
    if (!$stmt) {
        respond_error($conn, "Database error during password update preparation: " . $conn->error, 500);
    }
    
    // Bind parameters: string (s) for password, integer (i) for user_id
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            ...
            respond_error($conn, "User not found.", 404);
        }
    } else {
        respond_error($conn, "Error updating password: " . $stmt->error, 500);
    }
    $stmt->close();
    
// --- NEW ACTION: Delete User (Admin Only) ---
} elseif ($action === 'deleteUser' && $method === 'DELETE') {
    check_admin_auth($conn);
    
    $user_id = $data['userId'] ?? null;
    
    if (empty($user_id)) {
        respond_error($conn, "Missing user ID for deletion.", 400);
    }
    
    // CRITICAL SECURITY CHECK: Prevent admin from deleting their own account
    if ($_SESSION['user_id'] == $user_id) {
         respond_error($conn, "Cannot delete your own account.", 403);
    }

    // PATCH: Using prepared statements to prevent SQL Injection
    $stmt = $conn->prepare("DELETE FROM users WHERE id = ?");
    if (!$stmt) {
        respond_error($conn, "Database error during user deletion preparation: " . $conn->error, 500);
    }
    
    // Bind ID as integer (i)
    ...
    if ($stmt->execute()) {
        ...
        respond_error($conn, "Error deleting user: " . $stmt->error, 500);
    }
    $stmt->close();

// --- Registration Code Management Actions (Admin Only) ---
...
} elseif ($action === 'deleteCode' && $method === 'DELETE') {
    check_admin_auth($conn);
    
    $code = trim($data['code'] ?? '');
    
    if (empty($code)) {
        respond_error($conn, "Missing registration code for deletion.", 400);
    }
    
    // PATCH: Using prepared statements to prevent SQL Injection
    $stmt = $conn->prepare("DELETE FROM registration_codes WHERE code = ?");
    if (!$stmt) {
        respond_error($conn, "Database error during code deletion preparation: " . $conn->error, 500);
    }
    
    // Bind code as a string (s)
    $stmt->bind_param("s", $code); 

    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            http_response_code(200);
            echo json_encode(["message" => "Registration code deleted successfully"]);
        } else {
            respond_error($conn, "Registration code not found.", 404);
        }
    } else {
        respond_error($conn, "Error deleting registration code: " . $stmt->error, 500);
    }
    $stmt->close();
